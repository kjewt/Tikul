...
export const BtnEdit = ({ pw }: pwType) => {
    ...
    return (
        <>
            <button onClick={handleButtonClick} className="btn btn-primary btn-outline w-[330px]" aria-label="수정하기">
                수정하기
            </button>
            {isInputVisible && (
                <>
                    <form onSubmit={(e) => e.preventDefault()}>
                        ...
                        <button onClick={handlePWCheck} className="btn btn-primary w-[330px] mt-4" aria-label="수정">
                            확인
                        </button>
                    </form>
                    <dialog id="my_modal_3" className="modal">
                        <div className="modal-box">
                            <form method="dialog">
                                <button className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2" aria-label="닫기">✕</button>
                            </form>
